...
driven by GitHub Actions. Full details live in
`.github/instructions/devops-docker.instructions.md` and
`.github/instructions/ci-cd.instructions.md` (load via the `ci-cd` skill).

## Non-negotiables

- **Next.js standalone**: runtime image = `.next/standalone` + `.next/static` +
  `public`; `CMD ["node","server.js"]`; `HOSTNAME=0.0.0.0`, `PORT=3000`,
  `NODE_ENV=production`. Never `npm start` in the image.
- **Prisma 7 is binary-free in runtime** (`prisma-client` generator +
  `@prisma/adapter-pg`). `prisma generate` runs in the Docker build;
  `prisma migrate deploy` runs in CI/CD only. No engine binaries in the image.
- **`.env*` is gitignored and dockerignored.** `NEXT_PUBLIC_*` (Clerk) are
  passed as build args; everything else comes from compose `env_file`
  `.env.production` on the VPS and GitHub secrets in CI/CD.
- **Trigger.dev v4 only** (`trigger.dev@4.x`): `npx trigger.dev deploy` in CD
  with `TRIGGER_PROJECT_REF` + `TRIGGER_SECRET_KEY`. Never v3 `client.defineJob`.
