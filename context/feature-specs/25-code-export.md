...
- A 200 JSON failure response from `/api/export/code/[runId]/download?file=1` is shown as inline error feedback and is never treated as a ZIP download.
...
3.  Look up `ExportRun` by `runId`. Return 404 if not found.
4.  Verify `exportRun.userId === userId`. Return 403 if mismatch.
5.  Verify `exportRun.status === "completed"` and `exportRun.blobUrl` exists.
6.  For requests with `?file=1`, return a JSON status envelope with HTTP 200 while the export is pending or failed: `{ status: "pending" | "failed", error?: string }`.
7.  Fetch the ZIP from the blob URL when the export is completed.
8.  Return the ZIP buffer with headers: `Content-Type: application/zip`, `Content-Disposition: attachment; filename="{projectName}-{framework}.zip"`.

The client must inspect `Content-Type`, surface the reported `error` for a failed JSON response, and only call `blob()` for an archive response. A completed `?file=1` request preserves the `Content-Disposition` filename.
