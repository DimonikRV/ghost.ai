Add a small floating color toolbar so selected nodes can change both their background and tixt color directly on the canvas.
...
2. Add a toolbar above selected nodes.
   - only show when the node is selected (and not in editing mode)
...
- Nodes use prdefined background/text color pairs.
