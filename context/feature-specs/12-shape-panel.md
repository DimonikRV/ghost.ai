Add a bottom shape panel so user can drag shapes onto the canvas and create new nodes.
...
3. When dragging a shape, include the shape name and default size in the drag payload.
   ...
   - rectangles should be wider that tall
...
7. Add a basic render for the custom canvas node type so new nodes are visible.

   For this unit, render every shape as a simple bordered rectangle with the lable centered. Shape-specific visuals will be added later.
