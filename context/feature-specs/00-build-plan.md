# Build Plan

This file tracks the ordered list of units for this project.
Each unit maps to a spec file in this directory.

## Units

| # | Spec | Name | Status | Notes |
|---|------|------|--------|-------|
| 01 | 01-design-system.md | Design System | ✅ Done | shadcn/ui initialized, dark theme, cn helper |
| 02 | 02-editor.md | Editor Chrome | ✅ Done | Navbar + sidebar + dialog pattern |
| 03 | 03-auth.md | Authentication | ✅ Done | Clerk wired, route protection, auth pages |
| 04 | 04-project-dialogs.md | Project Dialogs | ✅ Done | Create/rename/delete dialogs, sidebar actions |
| 23 | 23-ci-cd-hardening.md | CI/CD Pipeline Hardening | ✅ Done | P0–P2 remediation, one item at a time |
| 24 | 24-test-coverage.md | Test Coverage Enhancement | 🔄 In Progress | 80% line coverage target, Vitest + Codecov CI |

## How to Add a Unit

1. Add a row to the table above
2. Create a corresponding spec file: NN-slug.md
3. Update progress-tracker.md when starting and completing